export interface InvitationFindAllDto {
...
/**
 * Valida o DTO para selecionar todos os convites
 */
export function validateInvitationFindAllDto(
  data: unknown,
): InvitationFindAllDto {
  ...
  if (dto.PE_STATUS !== undefined && dto.PE_STATUS !== null) {
    if (typeof dto.PE_STATUS !== "string") {
      throw new Error("PE_STATUS deve ser uma string");
    }
    if (dto.PE_STATUS.length > 191) {
      throw new Error("PE_STATUS não pode exceder 191 caracteres");
    }
  }
  ...
  return {
    ...
    PE_STATUS:
      typeof dto.PE_STATUS === "string" ? dto.PE_STATUS.trim() : undefined,
    PE_SEARCH_USER:
